    ...
    <div className="flex flex-col gap-6 min-w-0">
      ...
      <div className="bg-surface-container-lowest dark:bg-slate-900 rounded-[28px] p-4 sm:p-6 border border-surface-container dark:border-slate-800 shadow-stitch-md flex flex-col gap-4 min-w-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 min-w-0">
          {userRoles.map(usr => (
            <div key={usr.id} className="p-4 rounded-2xl bg-surface-container dark:bg-slate-800 flex items-center gap-3 min-w-0">
              <img src={usr.avatar} alt={usr.name} className="w-10 h-10 min-w-[40px] min-h-[40px] rounded-full object-cover flex-shrink-0" />
              <div className="min-w-0 flex-1">
                <span className="font-bold text-sm text-on-surface dark:text-slate-100 truncate block">{usr.name}</span>
                <p className="text-xs font-semibold text-primary dark:text-indigo-400 truncate">{usr.role}</p>
                <p className="text-[11px] text-outline dark:text-slate-400 truncate">{usr.email}</p>
              </div>
            </div>
          ))}
        </div>
