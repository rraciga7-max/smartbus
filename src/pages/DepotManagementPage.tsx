      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 min-w-0">
        {depots.map(dep => (
          <div
            ...
            className={`p-5 rounded-[24px] border cursor-pointer transition-all min-w-0 ${
          ...
          >
            <div className="flex justify-between items-start gap-2">
              <div className="min-w-0 flex-1">
                <span className={`text-[10px] font-bold uppercase tracking-wider truncate block ${selectedDepotId === dep.id ? 'opacity-80' : 'text-outline dark:text-slate-400'}`}>
                  {dep.code}
                </span>
                <h3 className="font-bold text-base mt-0.5 truncate">{dep.name}</h3>
              </div>
              <span className="material-symbols-outlined text-2xl flex-shrink-0">warehouse</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 bg-surface-container-lowest dark:bg-slate-900 rounded-[28px] p-6 border border-surface-container dark:border-slate-800 shadow-stitch-md flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs min-w-0">
            <div className="p-3.5 rounded-2xl bg-surface-container dark:bg-slate-800 min-w-0">
              <span className="text-outline dark:text-slate-400 font-bold uppercase text-[10px] truncate block">Maintenance Bays</span>
              <p className="text-lg font-bold text-amber-500 mt-1 truncate">{activeDepot.maintenanceBays} Bays</p>
            </div>
            <div className="p-3.5 rounded-2xl bg-surface-container dark:bg-slate-800 min-w-0">
              <span className="text-outline dark:text-slate-400 font-bold uppercase text-[10px] truncate block">Incoming Buses</span>
              <p className="text-lg font-bold text-blue-500 mt-1 truncate">{activeDepot.incomingBusesCount} En-Route</p>
            </div>
            <div className="p-3.5 rounded-2xl bg-surface-container dark:bg-slate-800 min-w-0">
              <span className="text-outline dark:text-slate-400 font-bold uppercase text-[10px] truncate block">Outgoing Buses</span>
              <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1 truncate">{activeDepot.outgoingBusesCount} Sched.</p>
            </div>
            <div className="p-3.5 rounded-2xl bg-surface-container dark:bg-slate-800 min-w-0">
              <span className="text-outline dark:text-slate-400 font-bold uppercase text-[10px] truncate block">Available Bays</span>
              <p className="text-lg font-bold text-primary dark:text-indigo-400 mt-1 truncate">{activeDepot.availableBays} Free</p>
            </div>
          </div>

          {/* Parking Utilization Chart Simulation */}
          <div className="p-5 rounded-2xl bg-surface-container-low dark:bg-slate-800/40 border border-surface-container dark:border-slate-800 flex flex-col gap-3 min-w-0">
            <h4 className="font-bold text-xs text-on-surface dark:text-slate-200 truncate">Hourly Bay Utilization Profile (Today)</h4>
            <div className="flex items-end justify-between gap-1 sm:gap-2 h-32 pt-4 px-1 sm:px-2 overflow-x-auto no-scrollbar">
        ...
        <div className="bg-surface-container-lowest dark:bg-slate-900 rounded-[28px] p-6 border border-surface-container dark:border-slate-800 shadow-stitch-md flex flex-col gap-4 min-w-0">
          ...
          <div className="flex flex-col gap-2.5">
            {parkedBuses.length === 0 ? (
              ...
              parkedBuses.map(b => (
                <div key={b.id} className="p-3 rounded-2xl bg-surface-container dark:bg-slate-800 flex items-center justify-between gap-2 text-xs min-w-0">
                  <div className="min-w-0 flex-1">
                    <span className="font-bold text-on-surface dark:text-slate-100 truncate block">{b.registrationNumber}</span>
                    <p className="text-[11px] text-outline dark:text-slate-400 truncate">{b.model} • Driver: {b.driverName}</p>
                  </div>
                  <span className="px-2 py-0.5 rounded-full font-bold text-[10px] bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 uppercase flex-shrink-0">
                    {b.status}
                  </span>
                </div>
              ))
