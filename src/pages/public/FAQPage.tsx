...
export const FAQPage: React.FC = () => {
  ...
  return (
    <div className="py-12 lg:py-16 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
      {/* PAGE HEADER */}
      ...
      <div className="relative max-w-2xl mx-auto">
        ...
        <input
          ...
          placeholder="Search questions (e.g. telematics, fuel theft, driver scores, GTFS)..."
          className="w-full pl-12 pr-4 py-4 rounded-2xl bg-surface-container-lowest dark:bg-slate-900 border border-surface-container dark:border-slate-800 text-on-surface dark:text-slate-100 text-sm font-medium outline-none focus:border-primary shadow-lg"
        />
        {searchQuery && (
          <button
        ...
        {filteredFaqs.length === 0 ? (
          ...
          filteredFaqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="rounded-3xl bg-surface-container-lowest dark:bg-slate-900 border border-surface-container dark:border-slate-800 overflow-hidden shadow-sm transition-all"
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? null : idx)}
                  className="w-full p-6 text-left flex items-center justify-between gap-4 font-bold text-on-surface dark:text-slate-100 hover:text-primary dark:hover:text-indigo-400 transition-colors"
                >
                ...
                {isOpen && (
                  <div className="px-6 pb-6 text-xs text-on-surface-variant dark:text-slate-300 leading-relaxed border-t border-surface-container/60 dark:border-slate-800 pt-4 animate-in fade-in duration-200">
                    {faq.a}
                  </div>
                )}
              </div>
